...
module AdlibPageSpecHelper
  ...
  def setup_three_children
    @page.save!
    AdlibPage.create!(:name => 'Foo', :layout => 'default', :slug => 'foo/').move_to_child_of @page
    AdlibPage.create!(:name => 'Bar', :layout => 'default', :slug => 'bar/').move_to_child_of @page
    AdlibPage.create!(:name => 'Baz', :layout => 'default', :slug => 'baz/').move_to_child_of @page
  end
end
...
describe AdlibPage do
  ...
  it "should know how to give a link to itself" do
    @page = valid_page_with_ancestors
    @page.save!
    @page.link_to_self.should == "<a href='/foo/bar/baz/'>Baz</a>"
  end

  it "should know how to give a link to itself (url)" do
    @page = valid_url_page
    @page.save!
    @page.link_to_self.should == "<a href='http://www.google.com/' target='_blank'>The Page Name</a>"
  end

  it "should know how to give a link to itself (shortcut)" do
    @page = valid_shortcut_page
    @page.save!
    @page.link_to_self.should == "<a href='bar/'>The Page Name</a>"
  end

  it "should know how to give breadcrumbs" do
    @page = valid_page_with_ancestors
    @page.save!
    @page.breadcrumbs.should == [ "<a href='/'>Root</a>",
                                  "<a href='/foo/'>Foo</a>",
                                  "<a href='/foo/bar/'>Bar</a>" ]
  end

  it "should know how to give child links" do
    setup_three_children
    @page.child_links.should == [ "<a href='bar/foo/'>Foo</a>",
